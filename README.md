...
- Память и контекст — AI запоминает важные факты между чатами
...
Раздел настроек, отвечающий за долговременную память AI между чатами. Память хранится локально на устройстве и автоматически подставляется в контекст при каждом новом сообщении.

### Настройки

| Настройка | По умолчанию | Описание |
|-----------|-------------|----------|
| **Enable memory** | Вкл | Общий выключатель памяти. Если выключен, AI не получает воспоминания и не сохраняет новые |
| **Save explicit memories** | Вкл | Сохранять факты, когда пользователь пишет «remember ...» или «запомни ...» |
| **Auto-summarize chats** | Выкл | Автоматически создавать краткую сводку после каждого ответа AI |
| **Summary TTL (days)** | 30 | Срок жизни автоматических сводок в днях (1–365). По истечении сводка удаляется |
| **Memory limit** | 8 | Максимальное количество воспоминаний, добавляемых в контекст каждого запроса |
| **Minimum importance** | 0.5 | Порог важности (0–1). Воспоминания с важностью ниже порога игнорируются |

### Как это работает

1. **Явное сохранение** — напишите в чате «запомни, что ...» или «remember that ...», и AI сохранит факт с высоким приоритетом (importance 0.9)
2. **Автосводки** — если включено, после каждого ответа AI генерирует краткую сводку разговора и сохраняет её со средним приоритетом (importance 0.6) и ограниченным сроком жизни
3. **Подстановка в контекст** — перед каждым запросом к AI из хранилища извлекаются наиболее релевантные воспоминания (отсортированные по важности и дате последнего использования) и добавляются как системное сообщение

### Управление памятью

- **View memories** — просмотр всех сохранённых воспоминаний с указанием типа и даты
- **Clear memory** — полная очистка всех воспоминаний
